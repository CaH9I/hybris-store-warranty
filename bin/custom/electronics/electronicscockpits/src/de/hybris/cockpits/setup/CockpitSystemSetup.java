 ...
 */
package de.hybris.cockpits.setup;
...
import de.hybris.cockpits.constants.ElectronicsCockpitsConstants;
 ...
 */
@SystemSetup(extension = ElectronicsCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(ElectronicsCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(ElectronicsCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						ElectronicsCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/electronicscockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
